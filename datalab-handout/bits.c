...
//   int ngmin = ~0x30 +1;
//   int ngmax = ~0x3a + 1;
//   return !((x + ngmin) >> 31) & !!((x + ngmax) >> 31);
// }
int isAsciiDigit(int x) {
  int upperCheck = (x + (~0x30 + 1)) >> 31;
  int lowerCheck = (x + (~0x3a + 1)) >> 31;
...
int howManyBits(int x) {
    ...
    int is_zero = !temp;
    int is_zero_mask = ((!is_zero) << 31) >> 31;
